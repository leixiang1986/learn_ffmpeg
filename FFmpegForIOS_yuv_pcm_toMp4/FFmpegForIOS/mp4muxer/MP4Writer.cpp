...
int MP4Writer::writerWriteYUV(void *data, int len) {
  
  return 0;
}

int MP4Writer::writerWritePCM(void *data, int len) {
...
int test(int argc, const char **argv)
{
    ...
    // 1. 打开yuv pcm文件
    const char *in_yuv_name = argv[0];
    const char *in_pcm_name = argv[1];
    const char *out_mp4_name = argv[2];
    FILE *in_yuv_fd = NULL;
    FILE *in_pcm_fd = NULL;
    //1. 打开测试文件
    // 打开YUV文件
