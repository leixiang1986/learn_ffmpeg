 ...
 * @LastEditTime: 2022-11-30 18:17:42
 * @LastEditors: xujin
 * @Reference: 
 */
#include "Muxer.h"
...
int GetSpsPpsData(char *buf, int buf_size, char *sps_pps, int &sps_pps_size) {
    ...
    while (nextStartCode = findNextStartCode(data + 3, buf_size - 3)) {
        ...
        if (startCode4(buf)) {
            startcode_size = 4;
        } else if (startCode3(buf)) {
            startcode_size = 3;
        }

        DBG("nalType:%d, frame_size:%d, starcode_size:%d", data[startcode_size] & 0x1f, frame_size, startcode_size);
        // sps
